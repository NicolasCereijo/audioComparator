...
img = librosa.display.specshow(decibels, x_axis = "time", y_axis = "log", ax = ax)
ax.set_title("Ejemplo de espectrograma", fontsize = 20)
...
fig, ax = plt.subplots(figsize=(10, 5))
img = librosa.display.specshow(decibelsMel, x_axis = "time", y_axis = "log", ax = ax)
ax.set_title('Ejemplo de espectrograma Mel', fontsize=20)
fig.colorbar(img, ax=ax, format=f'%0.2f')
plt.show()
